...
const NAV = [
  ...
  },
  {
    section: "Operations",
    items: [
      { href: "/warehouse/orders", label: "Orders Queue", icon: ClipboardList },
      { href: "/warehouse/stock", label: "Stock", icon: Package },
    ],
  },
  {
    section: "Catalogue",
    items: [
      { href: "/warehouse/products", label: "Products", icon: Store },
    ],
  },
];
      ...
      <div className="p-3 border-t">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium truncate">{user?.name ?? "Warehouse"}</p>
...
export default function WarehouseLayout({ children }: { children: React.ReactNode }) {
  ...
  const currentLabel =
    NAV.flatMap((s) => s.items).find((i) => pathname.startsWith(i.href))?.label ??
    "Warehouse";
